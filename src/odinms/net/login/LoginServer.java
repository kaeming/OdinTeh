...
import odinms.net.login.remote.LoginWorldInterface;
import odinms.net.mina.MapleCodecFactory;
import odinms.net.world.remote.WorldLoginInterface;
...
public class LoginServer implements Runnable, LoginServerMBean {
	...
	static {
		...
		try {
			mBeanServer.registerMBean(instance, new ObjectName("odinms.net.login:type=LoginServer,name=LoginServer"));
		} catch (Exception e) {
		...
		try {
			...
			synchronized (lwi) {
				...
				synchronized (wli) {
					...
					try {
						FileReader fileReader = new FileReader(System.getProperty("odinms.login.config"));
						...
						wli = worldRegistry.registerLoginServer(initialProp.getProperty("odinms.login.key"), lwi);
						...
						userLimit = Integer.parseInt(prop.getProperty("odinms.login.userlimit"));
						try {
	...
	public void run() {
		...
		tMan.start();
		loginInterval = Integer.parseInt(prop.getProperty("odinms.login.interval"));
		tMan.register(LoginWorker.getInstance(), loginInterval);
		rankingInterval = Long.parseLong(prop.getProperty("odinms.login.ranking.interval"));
