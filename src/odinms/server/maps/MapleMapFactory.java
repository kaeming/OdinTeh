...
import odinms.server.PortalFactory;
import odinms.server.life.AbstractLoadedMapleLife;
	...
	public MapleMap getMap(int mapid, boolean respawns, boolean npcs, boolean reactors) {
		...
		if (map == null) {
			synchronized (this) {
				...
				if (channel > 0 && Boolean.parseBoolean(ChannelServer.getInstance(channel).getProperty("odinms.world.faekchar"))) {
